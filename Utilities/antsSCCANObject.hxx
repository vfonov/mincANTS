...
::CGSPCA(unsigned int n_vecs )
{
  ...
  for (  unsigned int colind = 0; colind < n_vecs; colind++ )  
    ...
    while ( keepgoing )
      {
      VectorType x_k2 = x_k;
      RealType minerr2 = this->SparseNLConjGrad( this->m_MatrixP, x_k2, bp, 1.e-1, 10, true , true , baseind , locind  );
      keepgoing = false;
...
::NetworkDecomposition(unsigned int n_vecs )
{
  ...
  for(  unsigned int colind = extra_cols; colind < this->m_VariatesP.cols(); colind++ )
    ...
    //    MatrixType Aext( A.rows() , A.cols() + rmat.cols() , 0 );    Aext.set_columns( 0 , A );    Aext.set_columns( A.cols() , rmat );
