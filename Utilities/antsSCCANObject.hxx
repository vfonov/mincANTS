...
// Arnoldi Iteration SVD/SPCA
  ...
  while ( loop < maxloop && convcrit > 1.e-8 ) {
    ...
    if ( !condition2 ) condition2=( loop > (maxloop-itoff+1) ||  convcrit < 1.e-7   );
  ...
  for ( unsigned int k=0; k<n_vecs; k++) {
    ...
    if ( loop == 0 ) ptemp=this->m_VariatesP.get_column(k);
    if ( fnp < 1 && loop > 1 ){
    ...
    //VectorType pveck  = pmod.transpose() * (  pmod * ( pmod.transpose() * ( pmod  * ptemp )  ) ) ; // double p-space
    ...
    if ( fnp < 1 ){
      ...
      if ( pveck_temp.two_norm() > 0 ) pveck=pveck_temp/pveck_temp.two_norm();
    }
  ...
  std::cout <<"Iteration: " << loop << " Eigenvals: " << this->m_CanonicalCorrelations << " Sparseness: " << fnp  << " convergence-criterion: " << convcrit <<  " vex " << vex << std::endl;
  loop++;
...
::ComputeSPCAEigenvalues(unsigned int n_vecs, TRealType trace)
{
  ...
  for ( unsigned int i=0; i < n_vecs ; i++ )
  {
    ...
    for ( unsigned int j=0; j < i ; j++ )
      ...
      else v=this->m_VariatesP.get_column(j);
      ...
      if (nspace)
        ...
        if ( vn > 0 && unorm >  0  ) ip=1-inner_product( u/unorm ,  v/vn );
      eigenvalue_i*=ip;
      }
    evalsum+=oeig;
