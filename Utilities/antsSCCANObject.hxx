...
::NetworkDecomposition(unsigned int n_vecs )
{
  ...
  while (  colind < this->m_VariatesP.cols()  )
    ...
    avgprederr += loerror;
    std::cout << "predictionerr," << lmerror << ",col," << colind  << " prediction " << soln << " true " << r_leave_out << " err " << loerror << std::endl;
    }
  }
  std::cout << " correlation " << this->PearsonCorr( this->m_CanonicalCorrelations , this->m_OriginalMatrixR.get_column( 0 ) ) << std::endl;
