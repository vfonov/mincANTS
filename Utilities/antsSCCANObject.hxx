...
TRealType antsSCCANObject<TInputImage, TRealType>
::SparseConjGrad( typename antsSCCANObject<TInputImage, TRealType>::VectorType& x_k , 
		  typename antsSCCANObject<TInputImage, TRealType>::VectorType  b , TRealType convcrit = 1.e-3 , unsigned int maxits = 5 )
{
  bool keeppos = false;
  bool debug = false;
  RealType fnp = this->m_FractionNonZeroP;
  ...
  while (  deltaminerr > 0 && approxerr > convcrit && ct < maxits ) 
    ...
    if ( approxerr < minerr ) { minerr = approxerr ; bestsol=( x_k1 ); best_alph = alph; }
    }
    ...
    lasterr = approxerr; 
    approxerr = r_k1.two_norm();
    if ( true ) std::cout << " r_k1 " << approxerr <<  " derr " << deltaminerr << std::endl;
    deltaminerr = ( lasterr - approxerr );
    // measures the change in the residual 
    RealType   beta_k = inner_product( r_k1 , r_k1 ) /  inner_product( r_k , r_k );
    if ( debug ) std::cout <<" beta_k " << beta_k << std::endl;
		  ...
		  typename antsSCCANObject<TInputImage, TRealType>::VectorType  b , TRealType convcrit = 1.e-3 , unsigned int maxits = 5 )
{
  ...
  for ( unsigned int j=0; j< b.size(); j++) if ( fabs(b(j)) <= this->m_Epsilon ) indicator(j,j)=0;
  ...
  while (  deltaminerr > 1.e-4 && minerr > convcrit && ct < maxits ) 
    ...
    if ( best_alph == maxalph )  best_alph = this->LineSearch( A , x_k , p_k, b , maxalph, maxalph*2 , keeppos );
