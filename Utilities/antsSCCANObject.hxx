  ...
  typename antsSCCANObject<TInputImage, TRealType>::VectorType  b_in , TRealType convcrit, unsigned int maxits)
{
  ...
  while (  deltaminerr > 0 && approxerr > convcrit && ct < maxits ) 
    ...
    if ( debug ) std::cout << " ConjGrad " << approxerr <<  " minerr " << minerr << std::endl;
    lasterr = approxerr; 
  ...
  for ( unsigned int i = 0 ; i < soln.size(); i++ ) solerr += vnl_math_abs( soln(i) - b_in(i) ) * solerrsize;
                 ...
                 TRealType a, TRealType b, TRealType c, TRealType tau )
{
  ...
  if ( this->EvaluateEnergy( A , x_k, p_k, bsol, x, keeppos ) < this->EvaluateEnergy( A , x_k, p_k, bsol, b, keeppos ) ) {
    if (c - b > b - a) return GoldenSection( A , x_k , p_k, bsol , minalph, maxalph , keeppos , b, x, c, tau);
    else return GoldenSection( A , x_k , p_k, bsol , minalph, maxalph , keeppos , a, x, b, tau);
  }
  else {
    if (c - b > b - a) return GoldenSection( A , x_k , p_k, bsol , minalph, maxalph , keeppos , a, b, x, tau);
    else return GoldenSection( A , x_k , p_k, bsol , minalph, maxalph , keeppos , x, b, c, tau);
  }
              ...
              TRealType minalph  , TRealType maxalph  , bool keeppos )
{
  ...
  for ( RealType eb = minalph; eb <= maxalph; eb = eb + step ) 
    ...
    if( e < minerr ) { minerr = e; ebracket = eb; bestalph = eb; }
    }
  return bestalph;
                    ...
                    typename antsSCCANObject<TInputImage, TRealType>::VectorType  b , TRealType convcrit, unsigned int maxits, bool keeppos)
{
  ...
  while (  deltaminerr > 1.e-4 && minerr > convcrit && ct < maxits ) 
    ...
    RealType iprk = inner_product( r_k , r_k );
    RealType alpha_k = iprk / alpha_denom ;
    if ( debug ) std::cout << " alpha_k " << alpha_k << " iprk " << iprk <<  " alpha_denom " << alpha_denom << std::endl;
    ...
    if ( debug ) std::cout <<" xk12n " << x_k1.two_norm() <<" alpha_k " << alpha_k << " pk2n " << p_k.two_norm() << " xk1-min " << x_k1.min_value() << std::endl;
    /** Probably the most important step if you want an interpretable map */
    this->SparsifyP( x_k1 , keeppos ); /*******sparse******/    
    if ( debug ) std::cout <<" xk12n " << x_k1.two_norm() <<" alpha_k " << alpha_k << " pk2n " << p_k.two_norm() << std::endl;
    ...
    if ( debug ) std::cout << " r_k12n " << r_k1.two_norm() << " r_k2n " << r_k.two_norm() << std::endl;
    if ( debug ) std::cout << " x_k2n " << x_k.two_norm() << " x_k12n " << x_k1.two_norm() << std::endl;
    if ( debug ) std::cout << " r_k1 " << minerr <<  " derr " << deltaminerr << " ba " << best_alph / alpha_k << std::endl;
    // measures the change in the residual --- this is the Fletcher-Reeves form for nonlinear CG
    // see Table 1.1 \Beta^FR in A SURVEY OF NONLINEAR CONJUGATE GRADIENT METHODS
    // in this paper's notation d => p,  g => r , alpha, beta, x the same , so y = rk1 - rk
    //    RealType   beta_k = inner_product( r_k1 , r_k1 ) /  inner_product( r_k , r_k ); // classic cg
...
//  original_b = original_b - original_b.mean();  original_b = original_b / original_b.two_norm();
  ...
  for ( unsigned int mm = 0; mm < n_vecs; mm++ ) 
    ...
    if ( v.max_value() <= 0 ) this->m_VariatesP.set_column( mm , v * (-1.0) );
...
::BasicSVD( )
{
  ...
  if (  ( this->m_MatrixP.cols() - 1 ) < n_vecs ) n_vecs = this->m_MatrixP.cols() - 1;
  this->m_CanonicalCorrelations.set_size(n_vecs);
  this->m_CanonicalCorrelations.fill(0);
  ...
  for (unsigned int kk=0;kk<n_vecs; kk++) 
    ...
    if ( kk < init.columns() )
      ...
      if ( fnp < 1  )
        {
        if ( this->m_KeepPositiveP ) this->ConstantProbabilityThreshold( initvneg , fnp , this->m_KeepPositiveP );  else
      ...
      if ( fnp < 1 )
        {
        if ( this->m_KeepPositiveP ) this->ConstantProbabilityThreshold( initvpos , fnp , this->m_KeepPositiveP );  else
      ...
      else this->m_VariatesP.set_column( svdct, initvpos );
      svdct++;
      }
    }
...
// Arnoldi Iteration SVD/SPCA
  ...
  while ( loop < maxloop && convcrit > 1.e-8 ) {
    ...
    for ( unsigned int k=0; k<n_vecs; k++) 
      ...
      if ( fnp < 1 )
        {
        if ( this->m_KeepPositiveP ) this->ConstantProbabilityThreshold( newsol , fnp , this->m_KeepPositiveP );  else
  ...
  this->SortResults( n_vecs );
  convcrit=( this->ComputeEnergySlope(vexlist, 5) );
  std::cout <<"Iteration: " << loop << " Eigenval_0: " << this->m_CanonicalCorrelations[0]  << " Eigenval_1: " << this->m_CanonicalCorrelations[1] << " Eigenval_N: " << this->m_CanonicalCorrelations[n_vecs-1] << " Sparseness: " << fnp  << " convergence-criterion: " << convcrit <<  " vex " << vex << std::endl;
  loop++;
  if (debug) std::cout<<"wloopdone"<<std::endl;
  }//opt-loop
  //  std::cout << " cluster-sizes " << this->m_ClusterSizes << std::endl;
...
::ComputeSPCAEigenvalues(unsigned int n_vecs, TRealType trace , bool orth )
{
  ...
  for ( unsigned int i=0; i < n_vecs ; i++ )
    ...
    for ( unsigned int j=0; j < i ; j++ )
      ...
      if ( v.two_norm() >  0 ) v = v / v.two_norm();
      double ip =  inner_product( u, v );
      ip = 1 - fabs( ip );
      if ( orth ) eigenvalue_i*=ip;
      }
