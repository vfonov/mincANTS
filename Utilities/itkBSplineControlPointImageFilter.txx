...
::GenerateData()
{
  ...
  for ( unsigned int i = 0; i < ImageDimension; i++)
    {
    this->m_NumberOfControlPoints[i] = this->GetInput()->GetLargestPossibleRegion().GetSize()[i];
    }
...
template <class InputImage, class TOutputImage>
typename BSplineControlPointImageFilter<InputImage, TOutputImage>
::ControlPointLatticeType::Pointer
BSplineControlPointImageFilter<InputImage, TOutputImage>
::GenerateOutputImageAt( PointType P )
{
  ArrayType order;
  unsigned int frontIndex = 0;
  unsigned int backIndex = ImageDimension-1;
  for ( unsigned int i = 0; i < ImageDimension; i++ )
    {
    if ( P[i] > 1e9 )
      {
  ...
  for ( int i = 0; i < ImageDimension; i++ )
    ...
    for ( int j = 0; j < i; j++ )
      {
      size[order[j]] 
        = this->GetInput()->GetLargestPossibleRegion().GetSize()[order[j]];
      }
    collapsedPhiLattices[i]->SetRegions( size );
    collapsedPhiLattices[i]->Allocate();
    }
  ...
  typename ControlPointLatticeType::Pointer image 
    = ControlPointLatticeType::New();
  typename ControlPointLatticeType::RegionType::SizeType size;
  typename ControlPointLatticeType::PointType origin;
  for ( unsigned int i = 0; i < ImageDimension; i++ )
    {
    if ( P[i] > 1e9 )
      {
      size[i] = this->GetSize()[i];
  ...
  image->SetSpacing( this->GetSpacing() );
  ...
  for ( It.GoToBegin(); !It.IsAtEnd(); ++It )
    ...
    for ( unsigned int i = 0; i < ImageDimension; i++ )
      ...
      if ( P[i] < 1e9 )
        {
        U[i] = static_cast<RealType>( totalNumberOfSpans[i] ) *
               static_cast<RealType>( P[i] - this->m_Origin[i] ) /
               ( static_cast<RealType>( this->m_Size[i] - 1 ) 
                 * this->m_Spacing[i] );
        }
      if( vnl_math_abs( U[i] - static_cast<RealType>( totalNumberOfSpans[i] ) )
    ...
    for ( int i = ImageDimension-1; i >= 0; i-- )
      {
      if ( U[order[i]] != currentU[order[i]] )
        {
        for ( int j = i; j >= 0; j-- )
          {
          this->CollapsePhiLattice( collapsedPhiLattices[j+1], 
            collapsedPhiLattices[j], U[order[j]], order[j] );
          currentU[order[j]] = U[order[j]];
          }
        break;
        }
      }
...
typename BSplineControlPointImageFilter<InputImage, TOutputImage>
  ::ControlPointLatticeType::Pointer
BSplineControlPointImageFilter<InputImage, TOutputImage>
::CalculateLatticeGradientFromPoints( typename PointSetType::Pointer points )
{
  typename ControlPointLatticeType::Pointer gradientLattice
    = ControlPointLatticeType::New();
  gradientLattice->SetRegions( this->GetInput()->GetLargestPossibleRegion() );
  gradientLattice->Allocate();
  typename ControlPointLatticeType::PixelType P;
  P.Fill( 0 );
  gradientLattice->FillBuffer( P );

  typename RealImageType::Pointer normalizationLattice
    = RealImageType::New();
  normalizationLattice->SetRegions(
    this->GetInput()->GetLargestPossibleRegion() );
  normalizationLattice->Allocate();
  normalizationLattice->FillBuffer( 0 );

  typename PointDataContainerType::ConstIterator It;

  It = points->GetPointData()->Begin();
  while ( It != points->GetPointData()->End() )
    {
    typename PointSetType::PointType point;
    PointDataType data;

    points->GetPoint( It.Index(), &point );
    for ( unsigned int i = 0; i < ImageDimension; i++)
      {
      point[i] -= this->m_Origin[i];
      point[i] /= ( static_cast<RealType>( this->m_Size[i] - 1 )
    ...
    for ( unsigned int i = 0; i < ImageDimension; i++ )
      {
      if( vnl_math_abs( point[i] - NumericTraits<RealType>::One )
            <= this->m_BSplineEpsilon )
        {
        point[i] = NumericTraits<RealType>::One - this->m_BSplineEpsilon;
        }
      p[i] = static_cast<RealType>( point[i] ) * static_cast<RealType>(
        this->GetInput()->GetLargestPossibleRegion().GetSize()[i]
        - this->m_SplineOrder[i] );
      }
    ...
    ImageRegionIteratorWithIndex<RealImageType> Itw( w,
      w->GetLargestPossibleRegion() );
    ...
    for ( Itw.GoToBegin(); !Itw.IsAtEnd(); ++Itw )
      ...
      if ( this->GetInput()->GetLargestPossibleRegion().IsInside( idx ) )
        {
        gradientLattice->SetPixel( idx, gradientLattice->GetPixel( idx )
          + B*It.Value() );
        normalizationLattice->SetPixel( idx,
          normalizationLattice->GetPixel( idx ) + B );
        }
      }
    ++It;
    }

//  ImageRegionIterator<ControlPointLatticeType> ItG( gradientLattice,
//    gradientLattice->GetLargestPossibleRegion() );
//  ImageRegionIterator<RealImageType> ItN( normalizationLattice,
//    normalizationLattice->GetLargestPossibleRegion() );
//  for ( ItG.GoToBegin(), ItN.GoToBegin(); !ItG.IsAtEnd(); ++ItG, ++ItN )
//    {
//    if ( ItN.Get() > 0.0 )
//      {
//      ItG.Set( ItG.Get() / ItN.Get() );
//      }
//    }

  return gradientLattice;
}
