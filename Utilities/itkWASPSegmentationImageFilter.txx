...
//  get local sum over all  classes with spatial prior
           for( unsigned int sn = 0; sn < this->m_NumberOfClasses; sn++ ) 
             if ( this->m_DistanceImages[sn] ) spprob=spprob-this->m_DistanceImages[sn]->GetPixel( ItO.GetIndex() ); 
           if  (spprob < 0) spprob=0;
...
//   unsigned int n = this->m_ElapsedIterations % this->m_NumberOfClasses;
    {
    ImageRegionIterator<RealImageType> ItS( this->m_SumProbabilityImage,this->m_SumProbabilityImage->GetRequestedRegion() );
...
/** Normalize probability images by total probability */
         for (unsigned int nn=0; nn < this->m_NumberOfClasses; nn++ ) 
           {
              this->m_PosteriorImages[nn]->SetPixel(ItS.GetIndex(),this->m_PosteriorImages[nn]->GetPixel(ItS.GetIndex())/sum);
           }
        n = ItO.Get()-1;	     
...
//	std::cout << " begin class param and label " << std::endl;
  ...
  maxLabels->FillBuffer( NumericTraits<LabelType>::Zero );
    ...
    ImageRegionIterator<RealImageType> ItS( this->m_SumProbabilityImage,this->m_SumProbabilityImage->GetLargestPossibleRegion() );
    ImageRegionIterator<RealImageType> ItP( maxProbabilityImage,maxProbabilityImage->GetLargestPossibleRegion() );
...
// get max dist 
      ...
      for( ItD.GoToBegin(); !ItD.IsAtEnd(); ++ItD )
        ...
        if( ItD.Get() < 0.0 )
          {
	    if (fabs(ItD.Get()) > maxdist) maxdist=fabs(ItD.Get());
          } 
       }
      for( ItD.GoToBegin(); !ItD.IsAtEnd(); ++ItD )
        {
...
//  get local sum over all  classes with spatial prior
           for( unsigned int sn = 0; sn < this->m_NumberOfClasses; sn++ ) 
             if ( this->m_DistanceImages[sn] ) spprob=spprob-this->m_DistanceImages[sn]->GetPixel( ItO.GetIndex() ); 
           if  (spprob < 0) spprob=0;
//	   if ( this->m_DistanceImages[3]->GetPixel( ItO.GetIndex() ) > 0.5  && n == 2 ) std::cout << " sprob " <<  this->m_DistanceImages[3]->GetPixel( ItO.GetIndex() )  << " gprob " << ItT.Get()  << " spprob " << spprob << std::endl;
         }
	 else 
         {
         ...
         for (unsigned int n=0; n < this->m_NumberOfClasses; n++ ) 
           {
              ...
              if (sum>0)
                this->m_PosteriorImages[n]->SetPixel(ItO.GetIndex(),this->m_PosteriorImages[n]->GetPixel(ItO.GetIndex())/sum);
           }
...
//  std::cout <<" the issue is that if we are 'inside' the tissue , should we not exclude other tissue probabilities? " << std::endl;
...
//        distance = 1.0;
      ...
      double denom= this->m_CurrentClassParameters[whichClass-1][1]*2;
      RealType likelihood =
      1./(sqrt(denom*3.1416 )) *  vcl_exp( -0.5 * vnl_math_sqr( ItI.Get() - mu ) / denom );
      double finalprob=likelihood * mrfPrior * prior;
      if( this->m_MRFSigmoidAlpha > 0.0 )
        finalprob=1./(1.+exp(-1.0*( finalprob - this->m_MRFSigmoidBeta)/ this->m_MRFSigmoidAlpha )); // a decision function
      ItP.Set( finalprob );
      }
      else ItP.Set(0);
    ++ItI;
...
::AmassDistancePriors()
{
  ...
  if ( this->m_DistanceImages.size()  == this->m_NumberOfClasses )  return;
...
// set smimg equal to the original input image * threshold output 
  ...
  return filter->GetOutput();
   ...
   return smimg;
   if ( this->m_ElapsedIterations == 0) return smimg;
          ...
          for( unsigned int n = 0; n < neighborhoodSize; n++ )
           {
            ...
            bool isInBounds = false;
            if ( this->GetOutput()->GetPixel(  ItN.GetIndex(n) ) == whichClass ) isInBounds=true;
            if ( isInBounds ) {
           ...
           if (ct > 0) {  smct++;  smmean+=mean/ct; smimg2->SetPixel(ItN.GetIndex(), mean/ct ); }
        }
        ++ItN;
     }
    for( ItI.GoToBegin(); !ItI.IsAtEnd(); ++ItI )  ItI.Set( smimg2->GetPixel(ItI.GetIndex() ) );
