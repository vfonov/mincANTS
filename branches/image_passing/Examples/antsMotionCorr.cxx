...
int ants_motion( itk::ants::CommandLineParser *parser )
{
  ...
  for( int currentStage = numberOfStages - 1; currentStage >= 0; currentStage-- )
	...
	metric = corrMetric;
      ...
      if( std::strcmp( whichTransform.c_str(), "affine" ) == 0 )
        ...
        try
          {
          dfObserver->Execute( displacementFieldRegistration, itk::StartEvent() );
	  ...
	  /*
        ...
        try
          {
          antscout << std::endl << "*** Running gaussian displacement field registration (sigmaForUpdateField = "
	...
	antscout <<" AA " << std::endl;
	  ...
	  */
      ...
      resampler->Update();
      antscout << " done resampling " << std::endl;
  ...
  totalTimer.Stop();
  antscout << std::endl << "Total elapsed time: " << totalTimer.GetMeanTime() << " averagemetric " << metricmean << std::endl;
    {
    ...
    std::string fnmp ;
    if( outputPrefix[0] == '0' && outputPrefix[1] == '1' )
      {
	fnmp = std::string("MOCOparams.csv") ;
      }
    else
      {
