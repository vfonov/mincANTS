...
// Arnoldi Iteration SVD/SPCA
  ...
  while( loop<maxloop && (convcrit)> 1.e-8 )
    ...
    ::ants::antscout << "Iteration: " << loop << " Eval_0: " << this->m_CanonicalCorrelations[0] << " Eval_N: "
              << this->m_CanonicalCorrelations[n_vecs - 1] << " Sp: " << fnp  << " conv-crit: " << convcrit <<  " vex " << vex
              << std::endl;
    loop++;
    if( debug )
      {
            ...
            unsigned int maxits)
{
  ...
  while(  deltaminerr > 0 && approxerr > convcrit && ct < maxits )
    ...
    for ( unsigned int icept = 0; icept < x_k.size(); icept++ ) othermeans += ( x_k1( icept ) * A.get_column( icept ).mean() );
...
::CGSPCA(unsigned int n_vecs )
{
  ...
  for (  unsigned int colind = 0; colind < n_vecs; colind++ )
    ...
    reconstruction_error = ( approxmat  / approxmat.frobenius_norm() - this->m_MatrixP / this->m_MatrixP.frobenius_norm() ).frobenius_norm() ;
...
::LASSO( unsigned int n_vecs )
{
  ...
  for ( unsigned int its = 0 ; its < this->m_MaximumNumberOfIterations; its++ )
	...
	ypred = ypred + xj * beta_lasso( j );
      ...
      if ( j % 1000 == 0 && debug )
	{
        ::ants::antscout << "progress: " << 100 * j / this->m_MatrixP.cols() <<  " lasso-loop " << its << " : " <<  besterr << " num_vars_added " << added << std::endl;
	}
      } 
