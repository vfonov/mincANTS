...
::FEMDiscConformalMap()
{
  ...
  m_SurfaceMesh=NULL;
  for (int i=0; i<7; i++) m_PoleElementsGN[i]=0;
...
::InBorder(typename FEMDiscConformalMap<TSurface,TImage,TDimension>::GraphSearchNodePointer g)
{
  ...
  if ( this->m_HelpFindLoop[ g->GetIdentity() ] > 0  ) return true;
	...
	if (dist > m_MaxCost && dist < m_MaxCost + 1.0) return true;
	return false;
...
::InDisc(typename FEMDiscConformalMap<TSurface,TImage,TDimension>::GraphSearchNodePointer g)
{
  ...
  for (unsigned int i=0; i<this->m_DiscBoundaryList.size(); i++) {
    if ( d <= this->m_DiscBoundaryList[i]->GetTotalCost() ) return true;
  }
  return false;
...
::FindSource(IndexType index)
{
  ...
  for(int i =0; i < numPoints; i++)
  {
    ...
    float dist = 0.0;
    for (int j=0;j<ImageDimension; j++) dist+=(pt[j]-(float)index[j])*(pt[j]-(float)index[j]);
    dist=sqrt(dist);
    if (dist < mindist)
      {
	mindist=dist;
...
::FindMeanSourceInLabel(unsigned int label )
{
  ...
  else std::cout << " got Labels " << std::endl;
...
::GetBoundaryParameterForSquare( unsigned int nodeid, unsigned int whichParam )
{
  ...
  for (unsigned int i=0; i<this->m_DiscBoundaryList.size(); i++) {
    ...
    if ( this->m_DiscBoundaryList[i]->GetIdentity() == nodeid ) nodeparam=totallength;
    tangentlength=0;
    for (unsigned int d=0; d<diff.Size(); d++) tangentlength+=diff[d]*diff[d];
...
//  std::cout <<" x " << x << " y " << y << " al " << arclength << std::endl;
  ...
  if ( whichParam == 0 ) return x;
  else if ( whichParam == 1  ) return y;
...
::GetBoundaryParameterForCircle( unsigned int nodeid, unsigned int whichParam )
{
  ...
  if ( whichParam == 0 ) return cos(arclength);
  else if ( whichParam == 1  ) return sin(arclength);
  else return arclength; 
...
::AddVertexToLoop(  )
{
  ...
  for (unsigned int i=0; i<this->m_DiscBoundaryList.size(); i++) {
    unsigned int nxt=( ( i+1 ) % this->m_DiscBoundaryList.size() );
    //   std::cout << " this->m_DiscBoundaryList[i]->GetLocation() " << this->m_DiscBoundaryList[i]->GetLocation() << std::endl;
    diff=this->m_DiscBoundaryList[i]->GetLocation()-this->m_DiscBoundaryList[nxt]->GetLocation();
    tangentlength=0;
    for (unsigned int d=0; d<diff.Size(); d++) tangentlength+=diff[d]*diff[d];
    totallength+=tangentlength;
  ...
  std::cout << " length " << totallength << " valid? " << isvalid << " entries " << this->m_DiscBoundaryList.size() << " gsz " << this->m_HelpFindLoop.size() << std::endl;
  ...
  if ( !BestG  ) { std::cout << " does not exist " << std::endl; return false; }
  if ( this->m_HelpFindLoop[ BestG->GetIdentity() ] != 0 ) { std::cout << " already done " << std::endl; return false;  }
  ...
  */
  ...
  for (unsigned int j=0; j<this->m_DiscBoundaryList.size(); j++) {
	...
	for ( unsigned long g = 0 ; g < this->m_HelpFindLoop.size(); g ++ ) 
	  {
	    if ( this->m_HelpFindLoop[g] != 0 ) this->m_HelpFindLoop[g]=-1;
	  }
	for ( unsigned int j = 0 ; j < this->m_DiscBoundaryList.size(); j ++ ) 
	  {
...
::FindLoopAroundNode( unsigned int j_in )
{
  ...
  this->m_DiscBoundaryList.assign(manifoldIntegrator->m_BoundaryList.begin(),
	 ...
	 for (int i=0; i<discParameterizer->GetGraphSize(); i++)
	   {
	     ...
	     if ( this->m_HelpFindLoop[i] > 0 )  discParameterizer->GetGraphNode(i)->SetUnVisited();
	 ...
	 for (int i=0; i<lastlooper->GetGraphSize(); i++)
	   {
	     ...
	     if ( this->m_HelpFindLoop[i] > 0 )  lastlooper->GetGraphNode(i)->SetUnVisited();
	 ...
	 std::cout << " another idea --- get two points far apart  then solve a minimization problem across the graph that gives the average value in 0 => 1 ... " << std::endl;
...
::LocateAndParameterizeDiscBoundary( unsigned int label , bool CheckCost )
{
  ...
  for ( unsigned int j=0; j<gsz ; j++) { 
	...
	if ( inb > 0 ) {  //	  std::cout <<  " Node is in boundary " << std::endl;
	  ...
	  for (unsigned int i = 0; i < manifoldIntegrator->GetGraphNode(j)->m_NumberOfNeighbors; i++)
	    {
	      if (  fabs( manifoldIntegrator->GetGraphNode(j)->m_Neighbors[i]->GetValue(3) - label ) < 0.5 && cost <= this->m_MaxCost ) {
		// CurrentNode is in the boundary 	  
		inb+=1; 
	      }
	    } // neighborhood 
	  if ( inb >= 2 && alreadyfound[j] == false ) { // need at least two neighbors with same label 
	    ...
	    if ( cost >  effectivemaxcost )  effectivemaxcost=cost; 
	  }
	}
      } // less than max cost 
    } // if node exists 
  } // gsz
  std::cout << " Boundary has " << this->m_DiscBoundaryList.size() << " elements with eff. max cost " <<  effectivemaxcost << std::endl;
  if ( CheckCost ) {
      ...
      for (unsigned int myi = 0; myi < bsz ; myi++)
	{
	  if (  this->m_DiscBoundaryList[myi] != this->m_DiscBoundaryList[rootind] ) 
		...
		{		  
		  if (  this->m_DiscBoundaryList[myi] == this->m_DiscBoundaryList[rootind]->m_Neighbors[n] && !alreadyfound[myi] ) { // its in the bndry 
		    ...
		    if ( oknode ) {
		      ...
		      std::cout <<" cur " <<  this->m_DiscBoundaryList[rootind]->GetLocation() << " next " <<  this->m_DiscBoundaryList[myi]->GetLocation() << " boundcount "  << boundcount << " of " << bsz  << "  curroot " << rootind << std::endl; 
		      lastroot=rootind;
		      rootind=myi; 
		      myi=bsz;
		    } 
	  ...
	  if (  boundcount >= bsz-1 ) paramdone=true; 
	  else if (  myi == (bsz-1)  ) {
	    boundcount--; 
	    rootind=lastroot;
...
{
  if (!m_SurfaceMesh) { std::cout << " NO MESH"; return false;}
  ...
  for(int i =0; i < numPoints; i++)
      ...
      if (this->InDisc(manifoldIntegrator->GetGraphNode(i)))
	{
      ...
      if (this->InBorder(manifoldIntegrator->GetGraphNode(i))) boundsz++;
    }
	  ...
	  if( this->InBorder(manifoldIntegrator->GetGraphNode(nodeid)) )
	    {
	      ...
	      if ( this->m_MapToCircle) 
 	      ...
 	      else   fixvalue=this->GetBoundaryParameterForSquare(nodeid,option);
...
// first declare the flat image
  ...
  for ( int its =0; its <= maxits; its++)
      ...
      for( ::itk::fem::Solver::NodeArray::iterator n = m_Solver.node.begin(); n!=m_Solver.node.end(); n++)
	{
	  ...
	  index[0]=(long int)(0.5+( 1.0+manifoldIntegrator->GetGraphNode((*n)->GN)->GetValue(0) )*(float)(sz-1)/2.);
	  index[1]=(long int)(0.5+( 1.0+manifoldIntegrator->GetGraphNode((*n)->GN)->GetValue(1) )*(float)(sz-1)/2.);
  ...
  for( ::itk::fem::Solver::NodeArray::iterator n = m_Solver.node.begin(); n!=m_Solver.node.end(); n++)
    {
      ...
      float x;
      if (x > 0) x=(maxu - U)/urange; else x = (minu - U)/urange;
      float y;
      if (y > 0) y=(maxv - V)/vrange; else y = (minv - V)/vrange;
