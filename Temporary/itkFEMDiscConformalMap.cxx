...
::FindLoopAroundNode( unsigned int j_in )
{
  ...
  if ( this->m_RootNode ) {
	...
	for (unsigned int i = 0; i < neighborlist[curnode]->m_NumberOfNeighbors; i++)
	  {
	    for ( unsigned int m=0; m<nct; m++ ) {
	      if ( nfound[m] == false ) canloop=true; 
	      if ( m != curnode && neighborlist[m] == neighborlist[curnode]->m_Neighbors[i] && nfound[m]==false && m != lastnode ){
