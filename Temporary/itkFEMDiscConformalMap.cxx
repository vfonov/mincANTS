...
// 	  double diff = m_Radius[dof] - m_MaxCost;
	  ...
	  //if (diff > 0) diff = 0 ;
	  	  if ( ct % 100 == 0) std::cout << " mrdof " <<  m_RealSolution[dof]  <<  " rad " << m_Radius[dof] << std::endl;
...
::FixPointsAlongRadialLine()
{
  ...
  for (int j=pathsz-1; j>=0; j--)
    {// just use some specific number to label these nodes
      ...
      if ( j!=(pathsz-1) && j!=0 ) manifoldIntegrator->GetPathAtIndex(j)->SetUnVisitable();
    }
...
//   unsigned int maxits=m_Solver.GetNumberOfDegreesOfFreedom(); // should be > twice ndofs
  ...
  for( ::itk::fem::Solver::NodeArray::iterator n = m_Solver.node.begin(); n!=m_Solver.node.end(); n++)
      ...
      for( unsigned int d=0, dof; (dof=(*n)->GetDegreeOfFreedom(d))!=::itk::fem::Element::InvalidDegreeOfFreedomID; d++ )
	{
	  ...
	  if (ct % 100 == 0) std::cout << " midof " <<  m_ImagSolution[dof]  <<  " dof " << dof << std::endl;
	}
      ct++;
    }
  ...
  for( ::itk::fem::Solver::NodeArray::iterator n = m_Solver.node.begin(); n!=m_Solver.node.end(); n++)
    {
      ...
      float rat1=dp/r; if (rat1 > 10) rat1=10;
      float rat2=r/dp; if (rat2 > 10) rat2=10;
      if ( rat1 > 1 ) totallengthdistortion+= rat1;
      if ( rat2 > 1 )   totallengthdistortion+= rat2;
      ...
      //      if (ct % 100 == 0) std::cout << " R "  << r << " expu " <<  expu << " Theta " << theta << " tld " << totallengthdistortion/(float)ct << " V " << V << " mult " << mult <<  std::endl;
...
//       float denom=sqrt(x*x+y*y);
      ...
      if (makesquare)
	{
      ...
      if (r < minu) minu=r;
      if (theta < minv) minv=theta;
      if (r > maxu) maxu=r;
      if (theta > maxv) maxv=theta;
