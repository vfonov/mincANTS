/*=========================================================================
  ...
  Module:    $RCSfile: itkFEMConformalMap.cxx,v $
  Language:  C++
...
vtkPolyData* vtkGenSmoothMesh(vtkPolyData* vds)
{
...
FEMConformalMap<TSurface,TImage,TDimension>
::FEMConformalMap()
{ 
  m_Sigma=2.0e-4;
  m_ParameterFileName = "";
...
template <typename TSurface, typename TImage, unsigned int TDimension>
bool FEMConformalMap<TSurface,TImage,TDimension>
::GenerateSystemFromVtkSurfaceMesh()
{
  if (!m_VtkSurfaceMesh) { std::cout << " NO MESH"; return false;}
...
//  std::cout << " input E ";  std::cin >> m->E;
  ...
  for(int i=0; i<m_SurfaceMesh->GetNumberOfPoints(); i++) 
  {
    ...
    if (m_Debug) std::cout << "Point: "  << i << " coords " << pt[0] << ", " << pt[1] << ", "<< pt[2] << std::endl;
  ...
  for(int i=0; i<m_SurfaceMesh->GetNumberOfCells(); i++) 
  {
    ...
    if (m_Debug) std::cout << " pt 1 id " << tp[0] << " pt 2 id " << tp[1] <<" pt 3 id " << tp[2] << std::endl;
    cells++;
...
::MapImageToSphere(TImage* img, float rad )
{
  ...
  while( !it.IsAtEnd() ) 
    ...
    rindex=it.GetIndex();
    float dist=0;
    for (int ii=0; ii < ImageDimension; ii++) dist+=(float)(rindex[ii]-center)*(float)(rindex[ii]-center);
    dist = sqrt(dist);
    if ( dist > rad) it.Set( backgroundValue ); else it.Set( 1 );
    ++it;
    }
  PointType*  pt_ptr;
  PointType   pt;
  ...
  for(  itk::fem::Element::ArrayType::iterator elt=m_Solver.el.begin(); elt!=m_Solver.el.end(); elt++) 
  {
      ...
      for(unsigned int f=0; f<ImageDimension; f++)
        ...
        {
        Sol[f]= m_Solver.GetLinearSystemWrapper()->GetSolutionValue( 
              (*elt)->GetNode(nd)->GetDegreeOfFreedom(f) , 0);
        ...
        if (inimage ) m_SphereImage->SetPixel(sindex, img->GetPixel(rindex) + 1.0);//*100. +110.0);
        }
...
//  Float solval,posval;
  ...
  wimIter.GoToBegin();  
  for( ; !wimIter.IsAtEnd(); ++wimIter ) wimIter.Set(0.0);
...
::MapStereographicCoordinatesToImage(int cdim)
{
  ...
  for(  itk::fem::Element::ArrayType::iterator elt=m_Solver.el.begin(); elt!=m_Solver.el.end(); elt++) 
  {
      ...
      for(unsigned int f=0; f<ImageDimension; f++)
        ...
        if (inimage ) m_Image->SetPixel(rindex,  disp[cdim]+2.0);//*100. +110.0);
        }
...
//  itk::fem::LoadBC::Pointer l6;
  ...
  switch (absdim)
    ...
    case 2:
      for (int i=0; i<dofs; i++){
      if (m_FindingRealSolution) fixval=0; else fixval=oc;
    ...
    case 3:
      for (int i=0; i<dofs; i++){
      if (m_FindingRealSolution) fixval=0; else fixval=-1.0*oc; // was -2.0 
    ...
    case 4:
      for (int i=0; i<dofs; i++){
      if (m_FindingRealSolution) fixval=oc; else fixval=0.0;
    ...
    case 5:
      for (int i=0; i<dofs; i++){
      if (m_FindingRealSolution) fixval=-1.0*oc; else fixval=0.0;
...
//   this->FixPoles(oo);
  ...
  for (int j=0; j<m_Solver.GetNumberOfDegreesOfFreedom(); j++)
    ...
    if ( i != j) sum+=val;
//    if (val != 0) std::cout << " entry i,j " << i << " , " << j << "  =  " << m_Solver.GetLinearSystemWrapper()->GetMatrixValue(i,j)<<std::endl;
    }
...
{    
  ...
  for( ::itk::fem::Solver::NodeArray::iterator n = m_Solver.node.begin(); n!=m_Solver.node.end(); n++)
  {
    ...
    float temp;
    if (m_VtkSurfaceMesh) temp = scs->GetTuple1(idx);
      else temp = (float) image->GetPixel(index);
  ...
  if (!m_VtkSurfaceMesh) m_VtkSurfaceMesh=vtkGenSmoothMesh(polydata2);//polydata2;
