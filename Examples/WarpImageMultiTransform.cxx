...
int main(int argc, char **argv){
    ...
    if (argc<=3){
        std::cout << "WarpImageMultiTransform ImageDimension moving_image output_image [-R reference_image | --tightest-bounding-box] (--reslice-by-header) [--use-NN (use Nearest Neighbor Interpolator)]"
	...
	std::cout <<  " \n  the -R option defines the reference image space that you wish to warp INTO \n "
		  << " e.g. one could pass a smaller image in to force resampling of the output to the smaller image resolution " << std::endl;
        exit(0);
    }
