...
int main(int argc, char **argv){
    ...
    if (argc<=3){
      std::cout << argv[0] << " ImageDimension moving_image output_image -R fixed_image  MyWarp.nii.gz MyAffine.txt --use-NN (Nearest Neighbor Interpolator option) " << std::endl;
      std::cout << argv[0] << " ImageDimension fixed_image output_image -R moving_image  -i MyAffine.txt  MyInverseWarp.nii.gz --use-NN (Nearest Neighbor Interpolator option ) " << std::endl;
      std::cout << " you can also string together series of mappings --- e.g.:      MyAffine.txt MySecondAffine.txt  MyWarp.nii.gz MySecondWarp.nii.gz -i MyInverseAffine.txt    --- this can be an arbitrarily long series. " << std::endl;
      std::cout << " For this program, your moving_image will be either a 3-D image with vector voxels or a 4D image with scalar voxels. " << std::endl;
      std::cout << " You must define whether the image is 3/4D via the first parameter to the program " << std::endl;
      std::cout << " Output will be of the same type as input, but will be resampled to the domain size defined by the -R image ... " << std::endl;
