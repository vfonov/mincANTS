...
ProjectVector(typename TDisplacementField::PixelType invec, typename TDisplacementField::PixelType projvec )
{
  ...
  typename TDisplacementField::PixelType newvec;
  double ip=0;
  for (unsigned int i=0; i<ImageDimension; i++) ip+=invec[i]*projvec[i];
  for (unsigned int i=0; i<ImageDimension; i++) newvec[i]=ip*projvec[i];
  return newvec;
}
...
template <class TImage, class TDisplacementField>
void
ComputeJacobian(TDisplacementField* field,char* fnm, char* maskfn, bool uselog=false, bool norm=false, std::string projvec="")
{
  std::vector<std::string> v;
  if ( projvec.length() > 2 ) {
  ...
  if ( v.size() > 0 ) 
    {
    for (unsigned int i=0; i<ImageDimension;i++) pvec[i]=atof(v[i].c_str());
 ...
 for(  m_FieldIter.GoToBegin() ; !m_FieldIter.IsAtEnd(); ++m_FieldIter )
  {
    ...
    if ( v.size() > 0 ) cpix=ProjectVector<ImageType,FieldType>(cpix,pvec);
    for(unsigned int row=0; row< ImageDimension;row++)
      {
        ...
        if ( v.size() > 0 ) rpix=ProjectVector<ImageType,FieldType>(rpix,pvec);
        rpix = rpix*h+cpix*(1.-h);
        lpix = TransformVector<ImageType,FieldType>(field,difIndex[row][0]);
        if ( v.size() > 0 ) lpix=ProjectVector<ImageType,FieldType>(lpix,pvec);
...
int Jacobian(int argc, char *argv[])
{
  ...
  if ( argc < 3 )
  {
    std::cout << "Usage:   Jacobian gWarp outfile uselog maskfn normbytotalbool VectorToProjectWarpAgainst " << std::endl;
    std::cout <<" VectorToProjectWarpAgainst should be in the form 1.0x0.0x0.0 where x separates vector components " << std::endl;
    return 1;
  }
...
//  std::cout << " vecanidiffdone " << std::endl;
  ...
  std::string projvec;
  if (argc > 6) projvec=std::string(argv[6]);
...
int main(int argc, char *argv[])
{
  ...
  if ( argc < 3 )
  {
    std::cout << "Usage: " << argv[0] << " ImageDim gWarp outfile uselog maskfn normbytotalbool projectionvector " << std::endl;
    std::cout <<" for example " << std::endl << " ANTSJacobian 3  myWarp.nii   Output  1   templatebrainmask.nii   1 1x0 " << std::endl;
    std::cout << " the last 1 normalizes the jacobian by the total in the mask.  use this to adjust for head size. 1x0 will project the warp along direction 1,0 --- don't add this option if you dont want to do this " << std::endl;
    return 1;
  }
