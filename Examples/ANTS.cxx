...
//  while(argc--) printf("%s\n", *argv++);
    ...
    std::cout << " typical use, for a 3D image matching with mutual information : \n  ANTS  3  -m MI[fixedimage.nii,movingimage.nii,1,32] -o MutualInformationMapping.nii -i 30x20x0 -r Gauss[3,1] -t Elast[3] "  << std::endl;
    ...
    std::cout << "  3 levels of optimization with 30 iterations at the coarsest (fastest) level, 20 at the next and 0 at full resolution " << std::endl;
    std::cout << " \n typical use, for a 3D image matching with correlation : \n  ANTS  3  -m PR[fixedimage.nii,movingimage.nii,1,2] -o CC.nii -i 30x20x0 -r Gauss[3,0] -t SyN[1] "  << std::endl;
    std::cout << " the value 2 in the metric call ( -m PR[ ...] ) is the radius of the correlation window " << std::endl;
    std::cout << " the transformation is the greedy symmetric diffeomorphic mapping  with xgradient descent step-length of  1" << std::endl;
    std::cout << " \n typical use, for a 3D image matching with correlation implementation 2 : \n  ANTS  3  -m CC[fixedimage.nii,movingimage.nii,1,2] -o CC2.nii -i 30x20x0 -r Gauss[3,0.5] -t SyN[1,2,0.1] "  << std::endl;
    std::cout << " the value 2 in the metric call ( -m CC[ ...] ) is the radius of the correlation window " << std::endl;
    std::cout << " the transformation is the symmetric diffeomorphic mapping with time-varying velocity fied - 10 integration points in time and gradient step length of 1 " << std::endl;
