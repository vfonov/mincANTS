...
{
   std::cout << " Does not take into account reorientation needed when orientations change only in the header!! " << std::endl;
   std::cout << " consider the same DT image in 2 different orientations under an applied identity transform. "  << std::endl;
   std::cout << " the components will not be rotated correctly, but should be, b/c the header rotation is not accounted for in the reorientation filter."  << std::endl;
