...
int SetOrGetPixel(int argc, char *argv[])        
{
  ...
  unsigned int indy=0;
  if (ImageDimension >= 2) { indy=atoi(argv[argct]); argct++ ;}
  unsigned int indz=0;
  if (ImageDimension >= 3) { indz=atoi(argv[argct]); argct++ ;}
...
/** count the labels in the image */
 Iterator It ( image,image->GetLargestPossibleRegion() );
  ...
  for( it = myLabelSet.begin(); it != myLabelSet.end(); ++it )
    ...
    if (!valimage) std::cout <<" Volume Of Label " << *it << " is " << totalvolume <<   "  Avg-Location " << myCenterOfMass <<std::endl;
    else std::cout << " Volume Of Label " << *it << " is " << (unsigned long) totalvolume <<   "  Avg-Location " << myCenterOfMass <<" mass is " << totalmass << " average-val is " << totalmass/totalct << std::endl;
...
template<class TImage>
typename TImage::Pointer FitSmoothEstimateOfTissueIntensityFromPrior(
   typename TImage::Pointer  priorImage,  typename TImage::Pointer intensityImage, unsigned int splineOrder,
  unsigned int numberOfLevels  )
{
  typename TImage::DirectionType direction = intensityImage->GetDirection();
  typename TImage::DirectionType identity;
  identity.SetIdentity();
  intensityImage->SetDirection( identity );

  typedef float RealType;
  typedef itk::Vector<RealType, 1> ScalarType;
  typedef itk::Image<ScalarType, 
    ::itk::GetImageDimension<TImage>::ImageDimension> ScalarImageType;

  typedef itk::PointSet<ScalarType, 
    ::itk::GetImageDimension<TImage>::ImageDimension> PointSetType;
  ...
  itk::ImageRegionIterator<TImage> ItP( priorImage,
    priorImage->GetLargestPossibleRegion() );
  itk::ImageRegionIteratorWithIndex<TImage> ItI( intensityImage,
    intensityImage->GetLargestPossibleRegion() );
  for( ItI.GoToBegin(), ItP.GoToBegin(); !ItI.IsAtEnd(); ++ItI, ++ItP )
    {
    if( ItP.Get() > 0.0 )
      {
      typename TImage::PointType imagePoint;
      intensityImage->TransformIndexToPhysicalPoint(
        ItI.GetIndex(), imagePoint );
      typename PointSetType::PointType point;
      point.CastFrom( imagePoint );

      points->SetPoint( index, point );
      points->SetPointData( index, ItI.Get() );
      weights->InsertElement( index, ItP.Get() );

      index++;
      }
    }
  intensityImage->SetDirection( direction );

  unsigned int meshres=1;
  typename BSplineFilterType::ArrayType numberOfControlPoints;
  for( unsigned int d = 0; d < priorImage->GetImageDimension(); d++ )
    {
    numberOfControlPoints[d] = meshres + splineOrder;
    }
  ...
  bspliner->SetSize( intensityImage->GetLargestPossibleRegion().GetSize() );
  bspliner->SetOrigin( intensityImage->GetOrigin() );
  bspliner->SetDirection( intensityImage->GetDirection() );
  bspliner->SetSpacing( intensityImage->GetSpacing() );
  bspliner->SetGenerateOutputImage( true );
  bspliner->Update();

  typename TImage::Pointer output = TImage::New();
  output->SetRegions( intensityImage->GetLargestPossibleRegion() );
  output->SetOrigin( intensityImage->GetOrigin() );
  output->SetDirection( intensityImage->GetDirection() );
  output->SetSpacing( intensityImage->GetSpacing() );
  output->Allocate();

  itk::ImageRegionIterator<ScalarImageType> ItB( bspliner->GetOutput(),
    bspliner->GetOutput()->GetLargestPossibleRegion() );
  itk::ImageRegionIterator<TImage> ItO( output,
...
template <unsigned int ImageDimension>
int SegmentImage(      int argc, char *argv[])        
{
  ...
  std::string operation = std::string(argv[argct]);  argct++;
  std::string fn1 = std::string(argv[argct]);   argct++;
  unsigned int kclasses = 1; 
  if (argc > argct) kclasses=atoi(argv[argct]); argct++;
  float locweight=0.5;
  if (argc > argct) locweight=atof(argv[argct]); argct++;
  if (locweight < 0) locweight=0;  if (locweight > 1) locweight=1;
  float gwt=1.0-locweight;
  std::string priorfn1="";
  if (argc > argct) priorfn1=std::string(argv[argct]); argct++;
  std::string priorfn2="";
  if (argc > argct) priorfn2=std::string(argv[argct]); argct++;
  std::string priorfn3="";
  if (argc > argct) priorfn3=std::string(argv[argct]); argct++;
  if (kclasses < 1) kclasses=1;
  ...
  std::string kname=tempname+std::string("_seg")+extension;

  typename ImageType::Pointer image = NULL, p1image=NULL, p2image=NULL, p3image=NULL; 
  typename ImageType::Pointer kmeans = NULL,lkmeans=NULL; 
  ReadImage<ImageType>(image, fn1.c_str());
  typename ImageType::Pointer bimage=image;//N3BiasCorrectImage<ImageType>(image,image);

  if ( priorfn1.length() > 3 &&  priorfn2.length() > 3 &&  priorfn3.length() > 3 ) {
    ReadImage<ImageType>(p1image, priorfn1.c_str());
    ReadImage<ImageType>(p2image, priorfn2.c_str());
    ReadImage<ImageType>(p3image, priorfn3.c_str());
    ReadImage<ImageType>(lkmeans, priorfn3.c_str());
    lkmeans->FillBuffer(0);
    if ( kclasses == 4  ){
      Iterator vfIter2( image,  image->GetLargestPossibleRegion() );  
      for(  vfIter2.GoToBegin(); !vfIter2.IsAtEnd(); ++vfIter2 )  {
	if ( image->GetPixel(vfIter2.GetIndex() ) > 0 ) {
	  double px1=p1image->GetPixel(vfIter2.GetIndex() ); 
	  double px2=p2image->GetPixel(vfIter2.GetIndex() ); 
	  double px3=p3image->GetPixel(vfIter2.GetIndex() ); 
	  double total=px1+px2+px3; 
	  if ( total > 0) {	
	  px1/=total; px2/=total; px3/=total;
	  if ( px1 > px2 && px1 > px3 )
	    {
	    lkmeans->SetPixel(vfIter2.GetIndex() , 1);
	    float diff=px1-0.5;
	    if (diff < 0)  { px1-=diff;  px2+=diff*0.5;  px3+=diff*0.5; }  
	    }
	  else if ( px2 > px1 && px2 >= px3 )
	    {
	    lkmeans->SetPixel(vfIter2.GetIndex() , 2);
	    float diff=px2-0.5;
	    if (diff < 0)  { px2-=diff;  px1+=diff*0.5;  px3+=diff*0.5; }  
	    }
	  else if ( px3 > px1 && px3 > px2 )
	    {
	    lkmeans->SetPixel(vfIter2.GetIndex() , 3);
	    float diff=px3-0.5;
	    if (diff < 0)  { px3-=diff;  px2+=diff*0.5;  px1+=diff*0.5; }  
	    }        
          p1image->SetPixel(vfIter2.GetIndex() , px1); 
	  p2image->SetPixel(vfIter2.GetIndex() , px2); 
	  p3image->SetPixel(vfIter2.GetIndex() , px3); 
	  }
	}
      }
  ...
  if ( p1image && p2image && p3image ) {
    typename ImageType::Pointer p1mask = BinaryThreshold<ImageType>(0.5,1,1,p1image);
    typename ImageType::Pointer p2mask = BinaryThreshold<ImageType>(0.5,1,1,p2image);
    typename ImageType::Pointer p3mask = BinaryThreshold<ImageType>(0.5,1,1,p3image);
 ...
 //   WriteImage<ImageType>(p2image,"turd.nii.gz");
  ...
  kmeans = SegmentMRFKM<ImageType>(bimage,kmeans,kclasses, 1, 1);
  //std::cout << " done mrf " << std::endl;
  kmeans->SetDirection(image->GetDirection());
  kmeans->SetSpacing(image->GetSpacing());
  kmeans->SetOrigin(image->GetOrigin());
 // std::cout << " done set " << std::endl;
  }
  // build data term  images 
  for (unsigned int k=1; k<kclasses; k++) {
    typename ImageType::Pointer pimage = NULL;
    ReadImage<ImageType>(pimage, fn1.c_str());
    pimage->FillBuffer(0);
    typedef itk::ImageRegionIteratorWithIndex<ImageType> Iterator;
    Iterator vfIter2( pimage,  pimage->GetLargestPossibleRegion() );  
    float gestimatedmean=0,gestimatedvar=0;
    float lestimatedmean=0,lestimatedvar=0;
    unsigned long ct=0,lct=0;
    for(  vfIter2.GoToBegin(); !vfIter2.IsAtEnd(); ++vfIter2 )
      {
	double px=0;
	if (kmeans->GetPixel(vfIter2.GetIndex() ) == k )
	  {
	    px = bimage->GetPixel(vfIter2.GetIndex());
	    gestimatedmean+=px;
	    ct++;
	  }
        if (lkmeans) 
	if (lkmeans->GetPixel(vfIter2.GetIndex() ) == k )
	  {
	    px = bimage->GetPixel(vfIter2.GetIndex());
	    lestimatedmean+=px;
	    lct++;
    ...
    if ( ct == 0) { std::cout << " bad initialization " << std::endl; ct=1; }
	...
	if (kmeans->GetPixel(vfIter2.GetIndex() ) == k )
	  {
	    px = bimage->GetPixel(vfIter2.GetIndex());
	    float dist=(px -gestimatedmean);
	    gestimatedvar+=dist*dist;
	  }
        if (lkmeans) 
	if (lkmeans->GetPixel(vfIter2.GetIndex() ) == k )
	  {
	    px = bimage->GetPixel(vfIter2.GetIndex());
	    float dist=(px -lestimatedmean);
	    lestimatedvar+=dist*dist;
	  }
      }
    gestimatedvar/=(float)ct;
    ...
    for(  vfIter2.GoToBegin(); !vfIter2.IsAtEnd(); ++vfIter2 )  {
      double px = bimage->GetPixel(vfIter2.GetIndex());
      if ( k==1 && p1image ) estimatedmean=p1image->GetPixel(vfIter2.GetIndex())*locweight+gestimatedmean*gwt;
      if ( k==2 && p2image)  estimatedmean=p2image->GetPixel(vfIter2.GetIndex())*locweight+gestimatedmean*gwt;
      if ( k==3 && p3image ) estimatedmean=p3image->GetPixel(vfIter2.GetIndex())*locweight+gestimatedmean*gwt;
      double dataprob=exp(-1.0*(estimatedmean-px)*(estimatedmean-px)/(2.0*estimatedvar));
      // now regional probability MRF 
      double priorprob=0;
      typename ImageType::IndexType ind=vfIter2.GetIndex();
      GHood.SetLocation(ind);
      double kct=0.0,lkct=0.0;
      double tkct=(float)GHood.Size();
      for (unsigned int q = 0; q < GHood.Size(); q++)
	{ 
	  typename ImageType::IndexType ind2=GHood.GetIndex(q);
      	  if (kmeans->GetPixel(ind ) > 0 )
	    {
	      if (kmeans->GetPixel(ind2 ) == k ) kct++;
	    }
	  if (lkmeans)
      	  if (lkmeans->GetPixel(ind ) > 0 )
	    {
	      if (lkmeans->GetPixel(ind2 ) == k ) lkct++;
	    }
	}
      double finalprob=0;
      priorprob=kct/tkct;
      ...
      pimage->SetPixel( vfIter2.GetIndex(),finalprob);
    }
    
    std::ostringstream stm;
    stm << (k-1);
    std::string ltempname=tempname+std::string("_prob_")+stm.str()+extension;
    WriteImage<ImageType>(pimage,ltempname.c_str());
  }

  
  if ( kclasses == 4 ){
    Iterator vfIter2( image,  image->GetLargestPossibleRegion() );  
    std::string p1name=tempname+std::string("_prob_0")+extension;
    ReadImage<ImageType>(p1image, p1name.c_str());
    std::string p2name=tempname+std::string("_prob_1")+extension;
    ReadImage<ImageType>(p2image, p2name.c_str());
    std::string p3name=tempname+std::string("_prob_2")+extension;
    ReadImage<ImageType>(p3image, p3name.c_str());
    //loop over data
    for(  vfIter2.GoToBegin(); !vfIter2.IsAtEnd(); ++vfIter2 )  {
      if ( image->GetPixel(vfIter2.GetIndex() ) > 0 ) {
	double px1=p1image->GetPixel(vfIter2.GetIndex() ); 
	...
	if ( total > 0) {	
	  ...
	  if ( px1 > px2 && px1 > px3 )
	    {
	    kmeans->SetPixel(vfIter2.GetIndex() , 1);
	    float diff=px1-0.5;
	    if (diff < 0)  { px1-=diff;  px2+=diff*0.5;  px3+=diff*0.5; }  
	    }
	  if ( px2 > px1 && px2 >= px3 )
	    {
	    kmeans->SetPixel(vfIter2.GetIndex() , 2);
	    float diff=px2-0.5;
	    if (diff < 0)  { px2-=diff;  px1+=diff*0.5;  px3+=diff*0.5; }  
	    }
	  if ( px3 > px1 && px3 > px2 )
	    {
	    kmeans->SetPixel(vfIter2.GetIndex() , 3);
	    float diff=px3-0.5;
	    if (diff < 0)  { px3-=diff;  px2+=diff*0.5;  px1+=diff*0.5; }  
	    }        
          p1image->SetPixel(vfIter2.GetIndex() , px1); 
    ...
    std::cout <<" writing-norm2 " << tempname << std::endl;
    WriteImage<ImageType>(kmeans, kname.c_str());
    WriteImage<ImageType>(p1image, p1name.c_str());
    WriteImage<ImageType>(p2image, p2name.c_str());
    WriteImage<ImageType>(p3image, p3name.c_str());
  }

  }   
  //  WriteImage<ImageType>(kmeans,outname.c_str());
  return 0;
  
      //    std::cout << " Starting " << std::endl;
//    kmeans=BayesianSegmentation<ImageType>(image,kclasses,priorfn,nsmooth);
//    std::cout << "done - start MRF " << std::endl;
 
  typedef itk::BinaryThresholdImageFilter< ImageType, ImageType > ThresholdFilterType;
  typename ThresholdFilterType::Pointer threshold = ThresholdFilterType::New();  
  threshold->SetInput (kmeans);
  threshold->SetInsideValue(1);
  threshold->SetOutsideValue(0);
  threshold->SetLowerThreshold(kclasses-1);
  threshold->SetUpperThreshold(kclasses-1);
  threshold->Update();
...
int main(int argc, char *argv[])        
{
  ...
  { 
    ...
    std::cout << "  LabelStats labelimage.ext valueimage.nii ( compute volumes / masses of objects in a label image -- write to text file ) \n" << std::endl;
    std::cout << " DiceAndMinDistSum  LabelImage1.ext LabelImage2.ext OptionalDistImage  -- outputs DiceAndMinDistSum and Dice Overlap to text log file + optional distance image \n " << std::endl;
  ...
  switch ( atoi(argv[1]) )
   ...
   case 2:
     ...
     else if (strcmp(operation.c_str(),"Segment") == 0 )  SegmentImage<2>(argc,argv); 
   ...
   case 3:
     ...
     else if (strcmp(operation.c_str(),"Segment") == 0 )  SegmentImage<3>(argc,argv);
