...
//  WriteImage<ImageType>(Labels,(outname+"labels.nii.gz").c_str());
	  ...
	  if ( histogram[i] >= minSize)
	    {
	      ...
	      if ( i >= 0 && i < maximum) {  outf << sumofvalues[i]/(float)histogram[i] << ","; }
	      else {  outf << sumofvalues[i]/(float)histogram[i] << std::endl; }
	      std::cout << " Cluster " << i << " size  " << histogram[i] <<  " average " << sumofvalues[i]/(float)histogram[i] << " max " << maxvalue[i] << " label " <<  maxlabel[i] <<  std::endl;
	    }
	}
  ...
  else std::cout <<" File No Good! " << outname << std::endl;
  outf.close();
...
int main(int argc, char *argv[])        
{
  ...
  if ( argc < 4 )     
    { 
      ...
      std::cout << "Useage ex: \n  "<< std::endl; 
      std::cout << argv[0] << "  ImageDimension ROIMask.ext LabelImage.ext  OutPrefix   MinimumClusterSize  ValueImageThreshold  Image1WithValuesOfInterest.ext ...  ImageNWithValuesOfInterest.ext  \n \n " << std::endl;
