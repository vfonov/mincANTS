...
int ants_moco( itk::ants::CommandLineParser *parser )
{
  ...
  for( int currentStage = numberOfStages - 1; currentStage >= 0; currentStage-- )
    ...
    timer.Stop();
    std::cout << "  Elapsed time (stage " << currentStage+1 << "): " << timer.GetMeanTime() << " time-dim " << timedim <<  std::endl << std::endl;
    }
  }
...
//  option->SetUsageOption( 0, "Rigid[gradientStep]" );
  ...
  option->SetUsageOption( 3, "BSplineDisplacementField[gradientStep,updateFieldMeshSizeAtBaseLevel,totalFieldMeshSizeAtBaseLevel,<splineOrder=3>]" );
  option->SetUsageOption( 4, "TimeVaryingVelocityField[gradientStep,numberOfTimeIndices,updateFieldSigmaInPhysicalSpace,updateFieldTimeSigma,totalFieldSigmaInPhysicalSpace,totalFieldTimeSigma]" );
  option->SetDescription( description );
  parser->AddOption( option );
  }
