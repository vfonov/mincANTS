...
{\Large \bf An Open Source Framework for Multivariate $n$-Tissue
  Segmentation with Evaluation Using Open Data Sets} \\
\vspace*{0.5cm}
...
%manage a wide range of scalar and vector image datatypes.  
...
\footnote{
Consider $N$ sites each with a possible $K$ labels for a total of $N^K$ possible labeling configurations.  Due to the large $K$ associated with $K>>1$ problems, exact optimization is even more intractable than for the traditional 3-tissue scenario.
}
...
\end{align}
and the $(m,n)^{th}$ element of the covariance matrix, $\boldsymbol{\Sigma}$, is calculated from
\begin{align}
 \boldsymbol{\Sigma}_{mn,k} \leftarrow \frac{ \sum_{i=1}^N  ( \mathbf{y}_i - \boldsymbol{\mu}_k )^T p_k(l_k|\mathbf{y}_i) ( \mathbf{y}_i - \boldsymbol{\mu}_k )}{1 - \sum_{i=1}^N p^2_k(l_k|\mathbf{y}_i) }.
\end{align}
...
brainweb data as is but added a meta format data header to the raw
binary files.   An example copy of this header is contained in {\verb atroposBwebRF40FigureExample.sh }.  The 20 subject data, however, required excluding
