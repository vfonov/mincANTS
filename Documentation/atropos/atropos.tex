...
Expectation Maximization (EM) segmentation algorithm based on either parametric or non-parametric finite mixture modeling (FMM) of the
...
neuroimages into 69 neuronatomical regions to illustrate the practical
...
Registration approaches enable the ability to transform information
...
minimizing strategy typically shrinks run-time memory usage from a
...
study the effect of the MRF prior term and initialization strategy on
...
similar to those methods that fix segmentation parameters while
...
of bias on our N4 and Atropos algorithm and also the validity of our
...
Dice overlap, are shown in figure~\ref{fig:bweb1}.
\begin{figure}
\begin{center}
\begin{tabular}{c}
...
\caption{\baselineskip 12pt \small Brainweb single-subject results for
  each tissue.  The results show that the N4 bias correction
  algorithm, combined with Atropos, results in a minimal effect of
  bias, even at the 40\% level.  The optimal $\beta$ for the MRF term
  appears to be between $0.1$ and $0.2$.  The prior label image
  initialization is competitive with but inferior to K-means.}
\label{fig:bweb1}
\end{center}
\end{figure}
...
labeling results by converting the group labels to probability maps
and using them to drive many-class EM segmentation.  The procedure we
use here is similar to that described in \citep{Avants2010a}.  The
ground truth labels cover 69 classes and much of the brain.  Some
unlabeled regions remain which we assign to label 69 such that all
brain parenchyma contains a unique label.  The initialization of our
evaluation applies the script {\verb ants_multitemplate_labeling.sh }
(available in the ANTs toolkit) to the 19 Hammers evaluation
datasets located at \url{http://www.brain-development.org/} \citep{Hammers2003,Heckemann2006}.
These initial majority voting results are competitive with prior work
...
included with the ANTs toolkit.  {\bf \textcolor{red}{ FIXME}}.

\section{Discussion}
We introduced the Atropos algorithm, the theory and implementation
details and documented its performance in a variety of use cases.  We
also showed evidence that the openly available N4 bias correction
method can easily used with Atropos to improve segmentation.
...
\subsection{Performance on Single-Subject Brainweb Data}
How does Atropos performance compare to other methods that have been
evaluated on the basic brainweb data?   {\bf {\textcolor{red}{ FIXME}}}.

\subsection{Performance on Twenty-Subject Brainweb Data}
How does Atropos performance compare to other methods that have been
evaluated on the brainweb twenty data?   {\bf {\textcolor{red}{ FIXME}}}.
...
\subsection{Other Thoughts} {\bf {\textcolor{red}{ FIXME}}}
...
\section{Conclusion}
The Atropos algorithm is freely available to the public.  We release
...
strategy, alternatives to ICM and the specific method in which prior
