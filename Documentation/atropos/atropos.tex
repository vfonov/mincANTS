...
we use up to 68 classes) with a minimal memory footprint.  In addition
to describing the technical and implementation aspects of Atropos,
this work evaluates its performance on the parcellation problem given
two different training datasets, the BrainWeb dataset from Montreal
Neurological Institute and the Hammers atlas from UCL [FIXME].  We use
the datasets to evaluate three uses of the Atropos segmentation method: (1) K-means
segmentation without use of template data; (2) segmentation with prior
probability maps derived from a group template; (3) N-class EM
segmentation for parcellating the brain into 68 classes based on
training data.  These evaluation studies, combined with illustrative
examples that excercise Atropos options, demonstrate both performance
and wide applicability of this new segmentation tool.
