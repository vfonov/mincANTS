...
{\color{red}{We provide this capability to allow the user to implement
...
model parameters, {\color{red}{Atropos is capable of updating the current label 
...
filename in the \texttt{--icm [1]} set of options.  An example is given in 
Figure \ref{fig:r16icm}.  For each iteration, a random ordering of the
set $\{1,\ldots,C\}$ is created which prescribes the order in which the
...
a maximum number of these ``ICM iterations'' on the command line.}}
...
algorithm \citep{Tustison2010}.  N4 builds upon the popular
...
was 0.950--0.960 for WM, 0.930--0.950 for GM and 0.920--0.940 for CSF.
...
Dice overlap.  In that case, the SPM5 Dice overlap for BrainWeb single GM and WM is
0.885 and 0.909, respectively, while FreeSurfer and FSL's accuracy
is lower. The best GM result for 20 subjects is obtained by SPM5:
0.930; best WM Dice overlap is from FSL: 0.950.  We note that
...
{\color{red}{Atropos software is available in ANTs
