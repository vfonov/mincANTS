...
%work describes an open source, image-based algorithmic approach to
...
Expectation Maximization (EM) segmentation algorithm based on either parametric or non-parametric finite mixture modelings (FMM) of the
class intensities.  Atropos, available in ANTs
\footnote{
http://www.picsl.upenn.edu/ANTs
}, is capable of incorporating spatial prior probability maps,
(sparse) prior label maps and/or Markov Random Field (MRF) modeling
assumptions.  Atropos has also been efficiently implemented to handle
...
Related technological developments encompasses those driven by partial
volume modeling for increased accuracy in brain segmentation
...
\citep{Smith2007}.  Two recent comparative evaluation studies \citep{,Bresser2011} have shown that methods such as FAST,
...
The major contribution of the Atropos algorithm is a flexible,
well-designed, memory efficient implementation of core expectation maximization machinery
that is useful across many segmentation problems.  Atropos options
include different maximization criterion, an interactive mode, tunable
priors and multivariate density estimators using both
non-parametric and parametric functions.  To highlight the
...
segmentation utilize a user-selected observation model defining the
likelihood term and one or more prior probability terms.  The produce
...
covariance matrix, $\Sigma_k$.  Multivariate probabilities are derived from the
Mahalanobis distance.  An initial estimate for these model
...
Atropos enables two choices for the likelihood formulation.  One is
based on the classical finite mixture model (FMM) and the second is based on
the spatially varying mixture model (SVMM) defined in \citep{Sanjay-Gopal1998}.
...
pointed out by \cite{Zhang2001}, the exclusive use of the observation
models in modeling the intensity profile produces a less than optimal
...
descriptions below, we discuss our introduction of a third possible
...
FIXME DEFINE THE GAMMA --- SHOULD USE SVMM FROM HEBERT ?
...
template-based prior information at site $i$.  The user may also
choose mixing proportions equal to $\frac{1}{K}$ via the command line
interface to the posterior formulation.
...
\subsubsection{Supervised Initialization to the EM Algorithm}
PriorLabelImage
...
missing $\hat{\mathbf{x}}$ and subsequent re-estimation of the model
...
Atropos may therefore be written
\begin{eqnarray}
X \\ 
Y \\
Z
\end{eqnarray}
The solution will be a local maxima. 
...
dimension, size, origin, etc.), which enable multivariate likelihoods, can be specified 
...
observational model  \citep{Awate2006}.  
...
\input{evaluation_data_description.tex}
