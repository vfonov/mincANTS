...
single vector field that is constant in time.

By exponentiation of a constant velocity field, a diffeomorphism can
be generated from the following o.d.e (cf Equation (\ref{eq:ode}))
...
\mathbf{u}(\mathbf{x}) = \mathbf{u}(\mathbf{x}) + \delta  K \star {\bf U},
\end{eqnarray}
...
documentation \cite{ANTS2009}.
  
\subsection{ANTS Point-Set-Based Similarity Metrics}
In addition to intensity-based metrics, ANTS also contains similarity
metrics for registering point-sets.  These include a landmark matching
metric and two point-set metrics which can accommodate point-sets of
different cardinality.  These point-set metrics can be used alone for
strict point-set registration or in conjunction with intensity-based
metrics for dual intensity/point-set registration.
...
ANTS documentation provides the specific command used for the data.  
...
\label{fig:chalf}
\end{figure}
(via the {\verb --large-deformation } ~{\verb 1 } ~flag).  
...
between robustness and sensitivity for application-speciﬁc tailoring
\cite{Tustison2009a}.
