...
\noindent{\bf Warping and Invertibility.}ANTS uses physical space to define mappings. The origin etc is in the
coordinates of the world in which the picture (e.g. MRI) was
taken. Then, warping between physical coordinates is relatively
easy. Differences in bounding boxes etc present no problem -- assuming
you avoid inconsistent headers i.e. origins, directions, data
orientation. One may use PrintHeader to check the data and run simple,
fast tests (few iterations) to perform sanity checks before running
through loads of data.
...
Thus, in three-dimensional scenarios, there are fundamental limits to the 
...
ratio of volumes.  Thus, $\mathcal{J}(\x)=V_1(\phi(\x))/V_2(\x)$ where $V_i$ represents 
the volume in one domain versus the other.  Thus, if $\phi$ -- the mapping -- 
...
than the FA PR metric, 1.25 vs. 1.0.
...
\begin{verbatim}
ANTS 2 -o PSEtest  -i 91x70x55x40x30  -r Gauss[3,0.] -t SyN[0.3]  
   -m  PSE[Frown.nii,Smile.nii,Frown.nii,Smile.nii,1,0.2,11,0,25] 
    -m  MSQ[Frown.nii,Smile.nii,1,0] 
     --continue-affine 0  --number-of-affine-iterations 0 

WarpImageMultiTransform  2 Frown.nii temp.nii -R Smile.nii
   -i PSEtestAffine.txt PSEtestInverseWarp.nii 

WarpImageMultiTransform  2 Smile.nii temp2.nii -R Frown.nii 
