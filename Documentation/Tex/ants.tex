...
The displacement for each voxel is defined in physical image space.  If you want to 
...
image (TensorFA) and to perform Markov Random Field segmentation with
or without priors (Segment).   Many other operations are available.
...
TSYNWITHTIME=" -t SyN[0.5,5,0.01]  -r Gauss[3,0] "
TGREEDYSYN=" -t SyN[0.25]  -r Gauss[3,0] "
TELAST=" -t Elast[1] -r Gauss[0.5,3] " 
TEXP="  -t Exp[0.5,10] -r Gauss[0.5,3] " 
...
INVW=" -i ${NAME}Affine.txt ${NAME}InverseWarp.nii.gz "
FWDW=" ${NAME}Warp.nii.gz ${NAME}Affine.txt "
WarpImageMultiTransform  2 ${II} a.nii.gz -R ${JJ} $INVW
WarpImageMultiTransform  2 ${JJ} b.nii.gz -R ${II} $FWDW
\end{verbatim}
...
\verb  bash ~ \verb buildtemplateparallel.sh -h ~ to get usage. 
...
\end{enumerate}
The script offers an option ( -c 1  for qsub , -c 2 for voxbo )  so that you can send the whole thing to distributed computing. 
...
\begin{enumerate}
\item Preprocess images -- bias correction (e.g. via the \verb
  N4BiasFieldCorrection program), segmentation (via \verb Atropos ), etc.  Potentially construct an optimal template. 
