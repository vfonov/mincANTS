...
public:
  ...
  /**  Get the value and derivatives for single valued optimizers. */
  double GetValueAndDerivative2( IndexType index, 
  ...
  virtual CovariantVectorType  OpticalFlowUpdate(const NeighborhoodType &neighborhood, bool mov=false)
  {
    ...
    if (mov) fixedGradient= m_MovingImageGradientCalculator->EvaluateAtIndex( index )*(-1.0);
    ...
    double speedValue;
    if (mov ) speedValue = movingValue - fixedValue;
    else speedValue = fixedValue - movingValue;
  ...
  double ComputeLocalMutualInformation(IndexType imageindex, double fixedImageValue, double movingImageValue, bool deriv=false)
    {
  ...
  double ComputeProbabilities(IndexType oindex, unsigned int option=0)
	...
	//  mind[0]=movingImageParzenWindowTerm1;  
      ...
      if (option == 0) return jointPDFValue;
      else if (option == 1) return fixedImagePDFValue;
      else if (option == 2) return movingImagePDFValue;
...
//    double nccp1=0,nccm1=0;
	...
	if (denominator < 1.e-12)  denominator = 1.0;
	denominator=1.;
...
//	else std::cout << " no " << std::endl;
  //  if ( loce < this->m_RobustnessParameter) update.Fill(0);
	...
	if (this->m_MetricImage &&  
	    (unsigned int)oindex[0] == (unsigned int)this->Superclass::m_FixedImage->GetLargestPossibleRegion().GetSize()[0]-2 && 
	    (unsigned int)oindex[1] == (unsigned int)this->Superclass::m_FixedImage->GetLargestPossibleRegion().GetSize()[1]-2 )
	  { 
	    this->WriteImages();
	  }
      }
...
//    double nccp1=0,nccm1=0;
	...
	mag1=sqrt(mag1);
	if (mag1 > 1.e-5 ) movingGradientNorm/=mag1;  
	...
	for (int imd=0; imd<ImageDimension; imd++) update[imd]=sign*movingGradient[imd]/denominator*spacing[imd];
...
*/
 ...
 /** FixedImage image neighborhood iterator type. */
  ...
  void ComputePDFDerivatives( int pdfFixedIndex, int pdfMovingIndex,
			       CovariantVectorType& fixedImageGradientValue,
			      double cubicBSplineDerivativeValue ) const;

  void GetValueAndDerivative3(IndexType oindex,
			 MeasureType& value,
