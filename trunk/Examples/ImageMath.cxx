...
int PASL(int argc, char *argv[])
{
  ...
  for(  vfIter2.GoToBegin(); !vfIter2.IsAtEnd(); ++vfIter2 )
	...
	// where f is CBF, ΔM is thfef difference signal between tag and control acquisitions, λ (=0.9g/ml) is the blood/tiue water partition coefficient, T1a (1390 and 1650ms at 1.5/3T) is the longitudinal relaxation time of blood, α is the inversion efficiency, TI1 is the duration between the inversion and saturation pulses, α = 0.95 is the labeling efficiency, TI2 (= TI1 + w) is the image acquisition time.
	  //w is the post-labeling delay time and should include the acquisition time for each slices (Wi=w+(i-1)*Tacq), Tacq is the image acquisition time for each slice (35ms with the protocol recommended), i is the slice number. Tacq can be calculated by (MinTR- τ-w)/(slice number)
